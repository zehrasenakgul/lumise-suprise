...
class lumise_woocommerce {
    ...
    public function lumise_admin_notices() {
		...
		if( isset($addon_list) && !empty($addon_list) && count($addon_list) > 0 
			...
			if (!$key_valid_addon_bundle) {
				echo '<div class="wp-notice error" style="margin: 15px 0"><p>'.$lumise->lang('You must verify your purchase code for addon bundle to access to all features').'. <a href="'.admin_url('?page=lumise&nevnes-page=license').'">'.$lumise->lang('Enter your license now').'</a></p></div>';
			}
		...
		if(isset($addon_list) && !empty($addon_list) && count($addon_list) > 0 && isset($addon_list['vendors'])){
			...
			if (!$key_valid_addon_vendor) {
				echo '<div class="wp-notice error" style="margin: 15px 0"><p>'.$lumise->lang('You must verify your purchase code for addon vendor to access to all features').'. <a href="'.admin_url('?page=lumise&nevnes-page=license').'">'.$lumise->lang('Enter your license now').'</a></p></div>';
			}
		}
		...
		if(isset($addon_list) && !empty($addon_list) && count($addon_list) > 0 && isset($addon_list['printful'])){
			...
			if (!$key_valid_addon_printful) {
				echo '<div class="wp-notice error" style="margin: 15px 0"><p>'.$lumise->lang('You must verify your purchase code for addon printful to access to all features').'. <a href="'.admin_url('?page=lumise&nevnes-page=license').'">'.$lumise->lang('Enter your license now').'</a></p></div>';
			}
		}
	}
    ...
    public function menu_page() {
        ...
        add_submenu_page( 
        	'lumise', 
        	'Lumise'.(!empty($_GET['nevnes-page']) ? ' '. ucfirst($_GET['nevnes-page']) : ''), 
        	__( 'Dashboard', 'lumise' ),
        	'lumise_access', 
        	'lumise'
        );
        ...
        add_submenu_page( 
        	...
        	__( 'Orders', 'lumise' ),
        	'lumise_access', 
        	'admin.php?page=lumise&nevnes-page=orders'
        );
        ...
        add_submenu_page( 
        	...
        	__( 'Addons', 'lumise' ),
        	'lumise_access', 
        	'admin.php?page=lumise&nevnes-page=explore-addons'
        );
        ...
        add_submenu_page( 
        	...
        	__( 'Settings', 'lumise' ),
        	'lumise_access', 
        	'admin.php?page=lumise&nevnes-page=settings'
        );
	...
	public function plugin_row_meta($links, $file) {
		...
		if (LUMISE_PLUGIN_BASENAME == $file) {
			...
			$row_meta = array(
				'docs' => '<a href="' . esc_url( 'https://nevnes.com' ) . '" target=_blank aria-label="' . esc_attr__( 'View Lumise docs', 'lumise' ) . '">' . esc_html__( 'Documentation', 'lumise' ) . '</a>',
				'blog' => '<a href="' . esc_url( 'https://nevnes.com' ) . '" target=_blank aria-label="' . esc_attr__( 'View Lumise docs', 'lumise' ) . '">' . esc_html__( 'Lumise Blog', 'lumise' ) . '</a>',
	...
	public function submenu_file( $submenu_file ) {
		...
		$lp = isset($_GET['nevnes-page']) ? $_GET['nevnes-page'] : '';
		
		if ($p == 'lumise' && ($lp == 'addons' || $lp == 'explore-addons')) 
			return 'admin.php?page=lumise&nevnes-page=explore-addons';
		
		if ($p == 'lumise' && $lp == 'settings') 
			return 'admin.php?page=lumise&nevnes-page=settings';
			
		if ($p == 'lumise' && $lp == 'orders') 
			return 'admin.php?page=lumise&nevnes-page=orders';
