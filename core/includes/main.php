...
class lumise {
	...
	public function get_option($key = '', $default = '') {
		$activeLang = 'en';
		$activeLang = str_replace('\'','',$activeLang);
		if (empty($key))
			return $default;
		
		$query = sprintf(
			"SELECT `value` FROM `%s` WHERE `author`='%s' AND `key`='%s' AND `language_code`='".$activeLang."'",
	...
	public function set_option($key = '', $val = ''){
		...
		$query = sprintf(
			"SELECT `value` FROM `%s` WHERE `author`='%s' AND `key`='%s' AND language_code = '%s'",
			$this->db->prefix.'settings',
			$this->vendor_id,
            $this->lib->sql_esc($key),
			$activeLang
		...
		if (count($result) > 0) {
			$query = sprintf(
				"UPDATE `%s` SET `value`='%s' WHERE `author`='%s' AND `key`='%s' AND language_code = '%s'",
				$this->db->prefix.'settings',
	       ...
	       $this->db->rawQuery($query);
			...
			$query = sprintf(
				"INSERT INTO `%s` (`id`, `key`, `value`, `author`, `created`, `updated` , `language_code`) VALUES (NULL, '%s', '%s', '%s', '%s', '%s', '%s')",
	            $this->db->prefix.'settings',
	            $this->lib->sql_esc($key),
	            $this->lib->sql_esc($val),
	            $this->vendor_id,
