...
class lumise_views extends lumise_lib {
	...
	public function detail_header($args = array()) {
		...
		if (!empty($_GET['id'])) {
			echo '<h2><a href="'.$lumise->cfg->admin_url.'nevnes-page='.$args['pages'].(isset($args['type']) ? '&type='.$args['type'] : '').'">'.$args['pages'].'</a> <i class="fa fa-angle-right"></i> '.$args['edit'].'</h2>'.
					'<a href="'.$lumise->cfg->admin_url.'nevnes-page='.$args['page'].(isset($args['type']) ? '&type='.$args['type'] : '').(isset($_GET['callback']) ? '&callback=edit-cms-product' : '').'" class="add-new lumise-button"><i class="fa fa-plus"></i> '.
					$args['add'].
					'</a>';
		} else {
			echo '<h2><a href="'.$lumise->cfg->admin_url.'nevnes-page='.$args['pages'].(isset($args['type']) ? '&type='.$args['type'] : '').'">'.$args['pages'].'</a> <i class="fa fa-angle-right"></i> '.$args['add'].'</h2>';
		}

		echo $lumise_helper->breadcrumb(isset($_GET['nevnes-page']) ? $_GET['nevnes-page'] : '');
	...
	public function field_categories ($args) {
		...
		if (count($cates) > 0) {
			...
			<a href="<?php echo $lumise->cfg->admin_url; ?>nevnes-page=category&type=<?php echo $args['cate_type']; ?>" target=_blank class="lumise_add_cate">
		...
		<?php } else  { ?>
			...
			<?php if (!isset($args['create_new']) || $args['create_new'] !== false) { ?>
			<a href="<?php echo $lumise->cfg->admin_url; ?>nevnes-page=category&type=<?php echo $args['cate_type']; ?>" target=_blank  class="add-new">
	...
	public function field_multiple_images($attr = array()){
		...
		$productImages = $lumise->lib->get_rows('product_images', $search_filter, $orderby, array(), $per_page, $start, array('product_id'=>$_GET['id']), '');
		$jsonProductImages = json_encode($productImages['rows']);
		?>
		<script type="text/javascript" src="<?php echo $this->main->cfg->admin_assets_url;?>js/image-uploader.js"></script>
		<div class="img-preview">
			

			<div class="input-images" style="margin-bottom:10px"></div>
			<?php if (isset($attr['thumbn']) && isset($attr['thumbn_value'])) { ?>
		...
		<script>
			var json = '<?=$jsonProductImages?>';
			json = JSON.parse(json);
			var newData = [];
			for(var i = 0; i < json.length; i++){
				newData.push({
					id : json[i].id,
					src : 'http://localhost/data/'+json[i].image,
				})
			}
								...
								?>>
									...
									<div class="editzone-ranges" style="<?php if(isset($sdata->hide_size) && $sdata->hide_size == true){ echo 'display:none;'; } ?>">
										...
										<div class="edr-row" data-row="crop-marks">
											...
											<a href="#tip" class="tip">
												<i class="fa fa-question-circle"></i>
												<span><?php echo $this->main->lang('Show the guide line for crop marks and bleed in Nevnes editor.'); ?></span>
											</a>
										</div>
				...
				<header>
					<h3>
						...
						<button class="lumise-btn" data-act="samples">
							<i class="fa fa-th"></i> 
							<?php echo $this->main->lang('Nevnes samples'); ?>
						</button>
...
<b><a target="_blank" href="'.$this->main->cfg->admin_url.'nevnes-page=license"style="font-weight: 700; text-decoration: underline; font-style: italic;">Enter purchase code now</a></b></br>
<span>Notice: Each License can only be used for one domain.</br><a href="https://codecanyon.net/licenses/standard" target="blank" style="font-weight: 700; text-decoration: underline; font-style: italic;">Click to learn more about license term in Envato.</a></span>').'</font></p>';
			...
			if($key_valid) {
				$html .= '<a href="'.$url.'" target=_blank class="button">'.$this->main->lang('View in Nevnes editor').'</a>';
			}
