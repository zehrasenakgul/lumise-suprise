...
class lumise_admin extends lumise_lib
{
	...
	protected function process_save_data($field, $data)
	{
		...
		$pg = $lumise->esc('nevnes-page') . '-s';
		$pg = str_replace(array('s-s', '-s'), array('s', 's'), $pg);

		if ($lumise->esc('nevnes-page') == 'category' || $lumise->esc('nevnes-page') == 'tag')
			$pg = $_POST['type'];
		...
		if (isset($_POST['lumise-section'])) {
			...
			*/
			...
			if (isset($data['errors']) && count($data['errors']) > 0) {
				...
				if (!empty($data_id)) {
					$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=$section&id=$data_id&" . (isset($data['type']) ? '&type=' . $data['type'] : '') . (isset($_GET['callback']) ? '&callback=' . $_GET['callback'] : ''));
				} else {
					$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=$section" . (isset($data['type']) ? '&type=' . $data['type'] : '') . (isset($_GET['callback']) ? '&callback=' . $_GET['callback'] : ''));
				}
				exit;
			}
			...
			if (isset($id) && !empty($id)) {
				...
				$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=$section&id=$id" . (isset($data['type']) ? '&type=' . $data['type'] : '') . (!empty($_cb) ? '&callback=' . $_cb : ''));
		...
		if (isset($_POST['lumise-section'])) {
			...
			if (count($data['errors']) === 0) {
				...
				if (!isset($_POST['lumise-redirect']))
					$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=settings");
			...
			switch ($do_action) {
				...
				case 'do-update':
					...
					if ($key === null || empty($key) || strlen($key) != 36 || count(explode('-', $key)) != 5) {
						$this->main->set_option('purchase_key', '');
						echo '<script type="text/javascript">window.location.href = "' . $this->main->cfg->admin_url . 'nevnes-page=license";</script></body></html>';
						exit;
					} else if ($sys !== true) {
						...
						if ($data === 0) {
							...
							if ($res === TRUE) {
								...
								if ($this->main->connector->update()) {
									...
									$this->main->connector->set_session('lumise_msg', $lumise_msg);
									echo '<script type="text/javascript">window.location.href = "' . $this->main->cfg->admin_url . 'nevnes-page=updates";</script></body></html>';
									exit;
								} else {
									$lumise_msg['status'] = 'error';
	...
	public function check_caps($cap)
	{
		...
		if (
			...
			$this->main->connector->set_session(
				'lumise_msg',
				array(
					'status' => 'error',
...
class lumise_helper
{
	...
	public function breadcrumb($lumise_page, $type = null)
	{
		...
		$arr = $lumise->apply_filters('admin_breadcrumb', array(
			...
			'designs' => array(
				...
				'link'   => $lumise->cfg->admin_url . 'nevnes-page=designs',
				'child' => array(
					'design' => array(
						'type'   => '',
						'title'  => $lumise->lang('Design'),
						'link'   => $lumise->cfg->admin_url . 'nevnes-page=design',
					),
				),
			),
			'templates' => array(
		...
		foreach ($arr as $keys => $values) {
			...
			if ($keys == $lumise_page) {
				...
				$html .= '<li><a href="' . $lumise->cfg->admin_url . 'nevnes-page=dashboard">' . $lumise->lang('Dashboard') . '</a></li><li><span>' . $values['title'] . '</span></li>';
			}
			...
			if (isset($values['child'])) {
				...
				if (isset($values['child'][$lumise_page]) && $values['child'][$lumise_page]['type'] == $type) {
					...
					$html .= '<li><a href="' . $lumise->cfg->admin_url . 'nevnes-page=dashboard">' . $lumise->lang('Dashboard') . '</a></li><li><a href="' . $values['link'] . '">' . $values['title'] . '</a></li>';
				}
	...
	public function import_sample_shapes($shapes)
	{
		...
		$lumise->redirect($lumise->cfg->admin_url . 'nevnes-page=shapes');
	}
}
