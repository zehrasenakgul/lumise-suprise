<?php
    ...
    $config = array(
		...
		'total_page'    => $shares['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'nevnes-page=shares&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'nevnes-page=shares',
	);
	...
	<div class="lumise_content">
		...
		<?php if ( isset($shares['total_count']) && $shares['total_count'] > 0) { ?>
			...
			<div class="lumise_option">
				<div class="left">
					<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=shares" method="post">
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'nevnes-page=shares" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
