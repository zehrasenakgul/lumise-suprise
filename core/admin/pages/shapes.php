<?php
    ...
    $config = array(
		...
		'total_page'    => $shapes['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'nevnes-page=shapes&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'nevnes-page=shapes',
	);
...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			...
			<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=shape" class="add-new lumise-button">
				<i class="fa fa-plus"></i> 
				<?php echo $lumise->lang('Add new shape'); ?>
			</a>
			<?php if (isset($shapes['rows']) && count($shapes['rows']) < 3){ ?>
			<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=shapes&task=import-sample" class="add-new lumise-button">
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=shapes" method="post">
							...
							if ( is_array($shapes['rows']) && count($shapes['rows']) > 0 ) {
								...
								foreach ($shapes['rows'] as $value) { ?>
									...
									<tr>
										...
										<td><a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=shape&id=<?php if(isset($value['id'])) echo $value['id'] ?>" class="name"><?php if(isset($value['name'])) echo $value['name']; ?></a></td>
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'nevnes-page=shapes" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
