<?php
	...
	if (!empty($_POST['save_design'])) {
		...
		if (count($errors) == 0) {
			...
			$lumise->connector->set_session('lumise_msg', $lumise_msg);
			if (!empty($data_id)) {
				$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=design&id=".$data_id);
			} else {
				$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=design");
			}
			exit;
		...
		if (isset($id) && $id == true ) {
			$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=design&id=".$id);
			exit;
		}
	...
	<div class="lumise_content">
		<div class="lumise_header">
			<?php
				...
				if (isset($_GET['id'])) {
					echo '<h2>'.$lumise->lang('Edit Design').'</h2><a href="'.$lumise->cfg->admin_url.'nevnes-page=design" class="add-new lumise-button">'.$lumise->lang('Add New Design').'</a>';
				} else {
		...
		?>
		<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=design" method="post" class="lumise_form">
