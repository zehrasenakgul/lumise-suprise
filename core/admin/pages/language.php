	...
	<div class="lumise_content">
		...
		?>" id="lumise-clipart-form" method="post" class="lumise_form" enctype="multipart/form-data">
			...
			<div class="lumise_form_group lumise_form_submit">
				...
				<a class="lumise_cancel" href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=<?php echo $section; ?>s">
...
<?php
	...
	if (!empty($_POST['save_language'])) {
		...
		if (count($errors) == 0) {
			...
			$lumise->connector->set_session('lumise_msg', $lumise_msg);
			if (!empty($data_id)) {
				$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=language&id=".$data_id);
			} else {
				$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=language");
			}
			exit;
		...
		if (isset($id) && $id == true ) {
			$lumise->redirect($lumise->cfg->admin_url . "nevnes-page=language&id=".$id);
			exit;
		}

	}else if (count($langs) !== 0) {
		$errors['text'] = $lumise->lang('No language added, Please add new language before adding translate text').
		' &nbsp; <a href="'.$lumise->cfg->admin_url .'nevnes-page=languages">'.$lumise->lang('Languages').' <i class="fa fa-arrow-circle-right"></i></a>';
		$lumise_msg = array('status' => 'error', 'errors' => $errors);
		$lumise->connector->set_session('lumise_msg', $lumise_msg);
	}
	...
	<div class="lumise_content">
		<div class="lumise_header">
			<?php
				...
				if (!empty($data['id'])) {
					echo '<h2>'.$lumise->lang('Edit Translate Text').'</h2><a href="'.$lumise->cfg->admin_url.'nevnes-page=language" class="add-new lumise-button">'.$lumise->lang('Add New Language').'</a>';
				} else {
		...
		?>
		<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=language" method="post" class="lumise_form">
			...
			<div class="lumise_form_group lumise_form_submit">
				...
				<a class="lumise_cancel" href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=languages">
					<?php echo $lumise->lang('Cancel'); ?>
				</a>
			</div>
