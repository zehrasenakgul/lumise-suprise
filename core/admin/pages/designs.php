<?php
    ...
    $config = array(
		...
		'total_page'    => $designs['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'nevnes-page=designs&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'nevnes-page=designs',
	);
...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			...
			<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=design" class="add-new lumise-button"><?php echo $lumise->lang('Add New Design'); ?></a>
			<?php
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=designs" method="post">
							...
							if ( is_array($designs['rows']) && count($designs['rows']) > 0 ) {
								...
								foreach ($designs['rows'] as $value) { ?>
									...
									<tr>
										...
										<td><a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=design&id=<?php if(isset($value['id'])) echo $value['id']; ?>" class="name"><?php if(isset($value['name'])) echo $value['name']; ?></a></td>
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'nevnes-page=designs" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
