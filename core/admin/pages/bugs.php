<?php
    ...
    $config = array(
		...
		'total_page'    => $bugs['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'nevnes-page=bugs&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'nevnes-page=bugs',
	);
...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			...
			<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=bug" class="add-new lumise-button">
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=bugs" method="post">
							...
							if ( is_array($bugs['rows']) && count($bugs['rows']) > 0 ) {
								...
								foreach ($bugs['rows'] as $value) { ?>
									...
									<tr>
										...
										<td>
											<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=bug&id=<?php if(isset($value['id'])) echo $value['id'] ?>" class="name"><?php 
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'nevnes-page=bugs" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
