<?php
    ...
    $config = array(
		...
		'total_page'    => $products['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'nevnes-page=products&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'nevnes-page=products',
	);
...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			...
			<a href="<?php echo $lumise->cfg->admin_url; ?>nevnes-page=product" class="add-new lumise-button">
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=products" method="post">
				...
				<form class="less" action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=products" method="post">
							...
							if ( is_array($products['rows']) && count($products['rows']) > 0 ) {
								...
								foreach ($products['rows'] as $value) { ?>
									...
									<tr>
										...
										<td>
											<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=product&id=<?php if(isset($value['id'])) echo $value['id'] ?>" class="name"><?php echo isset($value['name']) ? $value['name'] : ''; ?></a>
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'nevnes-page=products" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
