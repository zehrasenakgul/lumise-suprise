<?php
    ...
    $config = array(
		...
		'total_page'    => $cate['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'nevnes-page=categories&type='.$type.'&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'nevnes-page=categories&type='.$type,
	);
...
<div class="lumise_wrapper">
	<div class="lumise_content">
		...
		<div class="lumise_header">
			...
			<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=category&type=<?php echo $type; ?>" class="add-new lumise-button">
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=categories&type=<?php echo $type ?>" method="post">
				...
				<form class="less" action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=categories&type=<?php echo $type ?>" method="post">
							...
							if (empty($data_search)) {
								...
								for ($i = $start; $i < $start + $per_page; $i++) {
									...
									<tr>
										...
										<td class="lumise-resource-title">
											<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=category&type=<?php echo $type; ?>&id=<?php if(isset($cate[$i]['id'])) echo $cate[$i]['id']; ?>" class="name"><?php if(isset($cate[$i]['name'])) echo str_repeat('&mdash;', $cate[$i]['lv']).$cate[$i]['name']; ?></a>
								...
								foreach ($cate['rows'] as $value) { ?>
									<tr>
										...
										<td><a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=category&id=<?php if(isset($value['id'])) echo $value['id']; ?>&type=<?php echo $type?>" class="name"><?php if(isset($value['name'])) echo $value['name']; ?></a></td>
										<td>
										<?php
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.$type.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'nevnes-page=categories&type='.$type.'" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
