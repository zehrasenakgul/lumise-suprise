<?php
    ...
    $config = array(
		...
		'total_page'    => $printings['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'nevnes-page=printings&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'nevnes-page=printings',
	);
...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			...
			<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=printing" class="add-new lumise-button">
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=printings" method="post">
							...
							if ( is_array($printings['rows']) && count($printings['rows']) > 0 ) {
								...
								foreach ($printings['rows'] as $value) { ?>
									...
									<tr>
										...
										<td>
											<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=printing&id=<?php if(isset($value['id'])) echo $value['id'] ?>" class="name"><?php if(isset($value['title'])) echo $value['title']; ?></a>
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'nevnes-page=printings" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
