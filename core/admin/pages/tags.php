<?php
    ...
    $config = array(
		...
		'total_page'    => $tags['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise->cfg->admin_url.'nevnes-page=tags&type='.$type.'&tpage={page}',
	    'link_first'    => $lumise->cfg->admin_url.'nevnes-page=tags&type='.$type,
	);
...
<div class="lumise_wrapper">
	<div class="lumise_content">
		<div class="lumise_header">
			...
			<a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=tag&type=<?php echo $type; ?>" class="add-new lumise-button"><?php echo $lumise->lang('Add New Tag'); ?></a>
			<?php
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise->cfg->admin_url;?>nevnes-page=tags&type=<?php echo $type; ?>" method="post">
					...
					<tbody>
						<?php foreach ($tags['rows'] as $value) {?>
								<tr>
									...
									<td><a href="<?php echo $lumise->cfg->admin_url;?>nevnes-page=tag&type=<?php echo $type; ?>&id=<?php if(isset($value['id'])) echo $value['id']; ?>" class="name"><?php if(isset($value['name'])) echo $value['name']; ?></a></td>
								</tr>
						<?php } ?>
					</tbody>
				</table>
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.$type.'data_search'] = '';
						echo '<a href="'.$lumise->cfg->admin_url.'nevnes-page=tags&type='.$type.'" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
